#!/usr/bin/python3
"""
A Flask program that gets weather status from an api and display it
"""
...
import requests
import datetime
from helper.functions import key, kelvin_to_degree, secret_key
from helper.functions import convert_time
...
def server_test():
    ...
    return jsonify({'message': 'Server is working fine.'}), 200
...
def search():
    ...
    # Raise Exception when user enters an invalid city or country name
    except KeyError:
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port='6556', debug=False)
